...
export class TestCaseGenerator {
    ...
    constructor(config) {
        const defaultConfig = {
            provider: (process.env.OLLAMA_HOST ? 'ollama' : 'openai'),
    ...
    async generateFromPRD(prdContent, options) {
    ...
    async generateFromGherkin(gherkinContent, options) {
    ...
    buildPRDPrompt(prdContent, options) {
...
Return only the test code, no explanations.`;
    ...
    buildGherkinPrompt(gherkinContent, options) {
        const framework = options?.framework || 'playwright';
        const language = options?.language || 'typescript';
